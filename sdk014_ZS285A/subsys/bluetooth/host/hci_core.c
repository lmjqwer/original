...
int bt_set_afh_host_channel(u8_t *channel)
{
	struct net_buf *buf;
	struct bt_hci_cp_set_afh_host_channel *cp;

	buf = bt_hci_cmd_create(BT_HCI_OP_SET_AFH_HOST_CHANNEL, sizeof(*cp));
	if (!buf) {
		return -ENOBUFS;
	}

	cp = net_buf_add(buf, sizeof(*cp));
	memcpy(cp->channel, channel, sizeof(cp->channel));
	return bt_hci_cmd_send(BT_HCI_OP_SET_AFH_HOST_CHANNEL, buf);
}
