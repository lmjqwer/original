...
/* Only for gatt profile pts */
void bt_keys_del_except_addr(const bt_addr_le_t *addr)
{
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(key_pool); i++) {
		...
		memset(&key_pool[i], 0, sizeof(struct bt_keys));
	}
}

void bt_keys_del_all(void)
{
	bt_keys_clear_all();
}

void bt_keys_del_addr(const bt_addr_le_t *addr)
{
	struct bt_keys *keys;
