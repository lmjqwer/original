...
u8_t hostif_bt_spp_get_rfcomm_channel(u8_t spp_id)
{
#ifdef CONFIG_BT_SPP
	int prio;
	u8_t ret;
	...
	prio = hostif_set_negative_prio();
	ret = bt_spp_get_rfcomm_channel(spp_id);
	hostif_revert_prio(prio);
...
int hostif_bt_set_afh_host_channel(u8_t *channel)
{
#ifdef CONFIG_BT_HCI
	int prio, ret;

	prio = hostif_set_negative_prio();
	ret = bt_set_afh_host_channel(channel);
	hostif_revert_prio(prio);
