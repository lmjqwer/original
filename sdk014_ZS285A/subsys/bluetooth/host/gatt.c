...
void gatt_read_group_rsp(struct bt_conn *conn, u8_t err,
				const void *pdu, u16_t length,
				void *user_data)
{
	...
	for (length--, pdu = rsp->data; length >= rsp->len;
		 ...
		 if (rsp->len == 6) {
			...
			u.u16.val = ((u16_t)(data->value)[0] + ((u16_t)(data->value)[1] << 8));
		 } else if (rsp->len == 20) {
		 	u.uuid.type = BT_UUID_TYPE_128;
		 	memcpy(u.u128.val, data->value, 16);
		 } else {
		 	//BT_ERR("unkonw uuid len: %d", rsp->len);
			BT_ERR("gatt:%d,%d\n", __LINE__, rsp->len);
		 }
...
int gatt_read_group(struct bt_conn *conn,
		  struct bt_gatt_discover_params *params) {
	 struct net_buf *buf;
	  ...
	  */
	 if (params->type == BT_GATT_DISCOVER_PRIMARY) {
	 	 val = sys_cpu_to_le16(BT_UUID_GATT_PRIMARY_VAL);
	 } else  if (params->type == BT_GATT_DISCOVER_SECONDARY){
	 	 val = sys_cpu_to_le16(BT_UUID_GATT_SECONDARY_VAL);
	 } else {
