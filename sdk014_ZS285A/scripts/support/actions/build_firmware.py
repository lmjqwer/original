...
CFG_FIRMWARE_NEED_PADDING_SDFS = True
    ...
    def encrypt_file(self, file_path, blk_size):
        ...
        if (is_windows()):
            ...
            if sys.version_info[0] >= 3:
                fw2x_path = script_path + '/utils/fw2x/fw2x.py'

        # extrace lfi.bin from btsys.fw
        if fw2x_path.find('fw2x.py') == -1:
            fw2x_cmd = [fw2x_path, '-fsssfff', encrypt_fw, 'MakeEncryptBin',\
                'Encrypt', str(blk_size), file_path, file_path, efuse_bin]
        else:
            fw2x_cmd = ['python', fw2x_path, '-i', file_path, '-o', file_path, '-b', str(blk_size), '-ef', efuse_bin]
