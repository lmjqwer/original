...
struct btsrv_scan_priv {
	u8_t user_discoverable:1;
	u8_t user_connectable:1;
	u8_t curr_discoverable:1;
	u8_t curr_connectable:1;
	u8_t curr_iac:2;
	u8_t srv_discoverable:1;
	u8_t srv_connectable:1;
...
static u8_t btsrv_scan_discoverable_connectable_to_mode(u8_t discoverable, u8_t connectable)
{
	u8_t mode = 0;
...
static void btsrv_scan_set_scan_mode(u8_t discoverable, u8_t connectable, u8_t iac_mode,
								struct bt_scan_parameter *param)
{
	u8_t req_scan_mode, curr_scan_mode;
	u8_t update_scan_mode = 0;
	u8_t update_inquiry_param = 0;
	u8_t update_page_param = 0;
	u8_t update_iac = 0;
...
static void btsrv_scan_check_state(void)
{
	...
	int dev_role;
	u8_t exp_discoverable = 0;
	u8_t ext_connectable = 0;
	u8_t iac_mode = INQUIRY_GIAC;
	u8_t max_phone_num;
	struct bt_scan_parameter param;
	...
	if (!p_scan->user_discoverable || !p_scan->srv_discoverable) {
		...
		exp_discoverable = 0;
	} else if ((dev_role == BTSRV_TWS_NONE) && (connected_dev_cnt == max_phone_num)) {
		exp_discoverable = 0;
	} else {
