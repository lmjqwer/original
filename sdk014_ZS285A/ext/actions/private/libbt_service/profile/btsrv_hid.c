...
static void hid_event_cb(struct bt_conn *conn, u8_t event,u8_t *data, u16_t len)
{
...
int btsrv_hid_send_rsp(u8_t status)
{
	struct bt_conn *conn = btsrv_rdm_hid_get_actived();
...
int btsrv_hid_process(struct app_msg *msg)
{
	...
	case MSG_BTSRV_HID_SEND_CTRL_DATA:
		...
		report->data = (u8_t*)report + sizeof(struct bt_hid_report);
