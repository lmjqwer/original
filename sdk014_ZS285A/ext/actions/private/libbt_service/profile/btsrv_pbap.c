...
struct btsrv_pbap_priv {
	...
	u8_t user_id;	/* ID from pbap client */
	char *path;
	u8_t connected:1;
...
static void *btsrv_pbap_find_priv_by_app_id(u8_t app_id)
{
	int i;
...
static void *btsrv_pbap_find_priv_by_user_id(struct bt_conn *conn, u8_t user_id)
{
	int i;
...
static void btsrv_pbap_connect_failed_cb(struct bt_conn *conn, u8_t user_id)
{
	btsrv_event_notify_ext(MSG_BTSRV_PBAP, MSG_BTSRV_PBAP_CONNECT_FAILED, conn, user_id);
}

static void btsrv_pbap_connected_cb(struct bt_conn *conn, u8_t user_id)
{
	btsrv_event_notify_ext(MSG_BTSRV_PBAP, MSG_BTSRV_PBAP_CONNECTED, conn, user_id);
}

void btsrv_pbap_disconnected_cb(struct bt_conn *conn, u8_t user_id)
{
	btsrv_event_notify_ext(MSG_BTSRV_PBAP, MSG_BTSRV_PBAP_DISCONNECTED, conn, user_id);
}

void btsrv_pbap_recv_cb(struct bt_conn *conn, u8_t user_id, struct parse_vcard_result *result, u8_t array_size)
{
...
static void btsrv_pbap_abort_get(u8_t app_id)
{
...
static void btsrv_pbap_connect_failed(struct bt_conn *conn, u8_t user_id)
{
	struct btsrv_pbap_priv *info = btsrv_pbap_find_priv_by_user_id(conn, user_id);
...
static void btsrv_pbap_connected(struct bt_conn *conn, u8_t user_id)
{
	struct btsrv_pbap_priv *info = btsrv_pbap_find_priv_by_user_id(conn, user_id);
...
static void btsrv_pbap_disconnected(struct bt_conn *conn, u8_t user_id)
{
	struct btsrv_pbap_priv *info = btsrv_pbap_find_priv_by_user_id(conn, user_id);
...
int btsrv_pbap_process(struct app_msg *msg)
{
	...
	case MSG_BTSRV_PBAP_ABORT_GET:
		btsrv_pbap_abort_get((u8_t)_btsrv_get_msg_param_value(msg));
		break;
