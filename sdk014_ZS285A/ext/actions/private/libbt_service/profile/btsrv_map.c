...
struct btsrv_map_priv {
	struct bt_conn *conn;
	u8_t app_id;	/* ID from bt manager */
	u8_t user_id;	/* ID from map client */
	u8_t folder_flags;
...
static void *btsrv_map_find_priv_by_app_id(u8_t app_id)
{
	u8_t i;
...
static void *btsrv_map_find_priv_by_user_id(struct bt_conn *conn, u8_t user_id)
{
	u8_t i;
...
static void btsrv_map_connect_failed_cb(struct bt_conn *conn, u8_t user_id)
{
	btsrv_event_notify_ext(MSG_BTSRV_MAP, MSG_BTSRV_MAP_CONNECT_FAILED, conn, user_id);
}

static void btsrv_map_connected_cb(struct bt_conn *conn, u8_t user_id)
{
	btsrv_event_notify_ext(MSG_BTSRV_MAP, MSG_BTSRV_MAP_CONNECTED, conn, user_id);
}

void btsrv_map_disconnected_cb(struct bt_conn *conn, u8_t user_id)
{
	btsrv_event_notify_ext(MSG_BTSRV_MAP, MSG_BTSRV_MAP_DISCONNECTED, conn, user_id);
}

static void btsrv_map_set_path_finised_cb(struct bt_conn *conn, u8_t user_id)
{
...
void btsrv_map_recv_cb(struct bt_conn *conn, u8_t user_id, struct parse_messages_result *result, u8_t array_size)
{
...
static void btsrv_map_abort_get(u8_t app_id)
{
...
static void btsrv_map_client_disconnect(u8_t app_id)
{
	struct btsrv_map_priv *info = btsrv_map_find_priv_by_app_id(app_id);
...
static void btsrv_map_client_get_folder_listing(u8_t app_id)
{
	struct btsrv_map_priv *info = btsrv_map_find_priv_by_app_id(app_id);
...
static void btsrv_map_connect_failed(struct bt_conn *conn, u8_t user_id)
{
	struct btsrv_map_priv *info = btsrv_map_find_priv_by_user_id(conn, user_id);
...
static void btsrv_map_connected(struct bt_conn *conn, u8_t user_id)
{
	struct btsrv_map_priv *info = btsrv_map_find_priv_by_user_id(conn, user_id);
...
static void btsrv_map_disconnected(struct bt_conn *conn, u8_t user_id)
{
	struct btsrv_map_priv *info = btsrv_map_find_priv_by_user_id(conn, user_id);
...
{   
	...
	case MSG_BTSRV_MAP_GET_FOLDERLISTING:
	    btsrv_map_client_get_folder_listing((u8_t)_btsrv_get_msg_param_value(msg));	    
	    break;	    
	...
	case MSG_BTSRV_MAP_ABORT_GET:
		btsrv_map_abort_get((u8_t)_btsrv_get_msg_param_value(msg));
		break;
	case MSG_BTSRV_MAP_DISCONNECT:
		btsrv_map_client_disconnect((u8_t)_btsrv_get_msg_param_value(msg));
		break;		
