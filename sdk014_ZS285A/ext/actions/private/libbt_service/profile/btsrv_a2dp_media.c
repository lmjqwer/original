...
static u16_t _btsrv_tws_sbc_cal_frame_len(u8_t *data)
{
	struct sbc_head *p_head = (struct sbc_head *)data;
	u8_t mode = p_head->mode;
	u8_t nrof_blocks = 4*(p_head->blocks + 1);
	u16_t nrof_subbands = 4*(p_head->subbands + 1);
	u16_t nbits = nrof_blocks * p_head->bitpool;
	u16_t result = nbits;
...
static int _btsrv_a2dp_sbc_parser_frame_info(u8_t *data, u16_t len, u16_t *f_len, u16_t *f_cnt)
{
	u8_t *pData = data;
	u16_t frame_num = 0;
	u16_t frame_len = len;
...
static int _btsrv_a2dp_aac_parser_frame_info(u8_t *data, u16_t len, u16_t *f_len, u16_t *f_cnt)
{
	u8_t *pData = data;
	u8_t temp;
	u16_t frame_num = 0;
...
static u32_t btsrv_a2dp_sbc_cal_frame_time_us(u8_t *data)
{
	struct sbc_head *p_head = (struct sbc_head *)data;
	u8_t nrof_blocks = 4*(p_head->blocks + 1);
	u16_t nrof_subbands = 4*(p_head->subbands + 1);
	u32_t frame_time, samples;
...
static u32_t btsrv_a2dp_aac_cal_frame_time_us(u8_t *data)
{
	u32_t samples, time_us;
...
static u16_t btsrv_a2dp_sbc_cal_frame_samples(u8_t *data)
{
	struct sbc_head *p_head = (struct sbc_head *)data;
	u16_t nrof_blocks = 4*(p_head->blocks + 1);
	u16_t nrof_subbands = 4*(p_head->subbands + 1);
...
static u8_t btsrv_a2dp_sbc_get_samples_rate(u8_t *data)
{
	struct sbc_head *p_head = (struct sbc_head *)data;
	u8_t samples;
...
static u8_t btsrv_a2dp_aac_get_samples_rate(u8_t *data)
{
	u8_t samples;
...
static const u8_t sbc_44k_zero_frame[109] = {
...
static const u8_t sbc_48k_zero_frame[109] = {
...
static u8_t *btsrv_a2dp_sbc_get_zero_frame(u16_t *len, u8_t sample_rate)
{
	u8_t *data;
	...
	case 44:
		*len = sizeof(sbc_44k_zero_frame);
		data = (u8_t *)sbc_44k_zero_frame;
		break;
	case 48:
		*len = sizeof(sbc_48k_zero_frame);
		data = (u8_t *)sbc_48k_zero_frame;
		break;
	default:
		*len = 0;
		data = NULL;
...
static const u8_t aac_zero_frame[38] = {
