...
#ifdef CONFIG_USER_VOLUME_SAVE
#include <thread_timer.h>
#include <nvram_config.h>

struct thread_timer vol_monitor_timer;

static void audio_system_flush_nvram_conifg_info(struct thread_timer *ttimer, void *expiry_fn_arg)
{
...
//这种断电方案 实时更新 音量
	...
	// nvram_config_set(name, temp, strlen(temp) + 1);
	// return 0;
#endif
	if (thread_timer_is_running(&vol_monitor_timer)) {
		thread_timer_stop(&vol_monitor_timer);
	}
	thread_timer_start(&vol_monitor_timer, 2000, 0);
	printk("-bbb\n");
#endif
...
#ifdef CONFIG_USER_ALL_MODE_VOLUME_PUBLIC
		audio_system->linein_volume = audio_system->music_volume = volume;
		_audio_system_save_volume(CFG_BTPLAY_VOLUME, volume);
#else
...
#ifdef CONFIG_USER_ALL_MODE_VOLUME_PUBLIC
		audio_system->lcmusic_volume = audio_system->music_volume = volume;
		_audio_system_save_volume(CFG_BTPLAY_VOLUME, volume);
#else
...
#ifdef CONFIG_USER_ALL_MODE_VOLUME_PUBLIC
		audio_system->fm_volume = audio_system->music_volume = volume;
		_audio_system_save_volume(CFG_BTPLAY_VOLUME, volume);
#else
...
#ifdef CONFIG_USER_ALL_MODE_VOLUME_PUBLIC
		audio_system->i2srx_in_volume = audio_system->music_volume = volume;
		_audio_system_save_volume(CFG_BTPLAY_VOLUME, volume);
#else
...
#ifdef CONFIG_USER_ALL_MODE_VOLUME_PUBLIC
		audio_system->spidf_in_volume = audio_system->music_volume = volume;
		_audio_system_save_volume(CFG_BTPLAY_VOLUME, volume);
#else
...
#ifdef CONFIG_USER_BT_BACKPLAY_DEBUG
int aduio_system_btmusic_to_btcall_flag(bool read_mode, bool flags)  //获取 或者 保存 蓝牙切换至通话时不暂停音乐的标志
{
	if(read_mode == READ_BTCALL_FLAG_DATA)
		return 	audio_system->btcall_flag;
	
	audio_system->btcall_flag = flags;
	return 0;
}
#endif
...
#ifdef CONFIG_USER_BT_BACKPLAY_DEBUG
	aduio_system_btmusic_to_btcall_flag(SAVE_BTCALL_FLAG_DATA, false);
#endif
...
#ifdef CONFIG_USER_VOLUME_SAVE
	thread_timer_init(&vol_monitor_timer, audio_system_flush_nvram_conifg_info, NULL);
#endif
	/* audio_flinger_init(); */
