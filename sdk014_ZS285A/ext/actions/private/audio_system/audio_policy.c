...
int audio_policy_get_record_left_channel_id(u8_t stream_type)
{
	...
	case AUDIO_STREAM_USOUND:
		...
		channel_id = AUDIO_ANALOG_AMIC0_LINE_IN0;
#else
		channel_id = AUDIO_ANALOG_MIC0;
#endif
	break;
	case AUDIO_STREAM_VOICE:
#ifdef CONFIG_AEC_REF_HW_MODE
		channel_id = AUDIO_ANALOG_AMIC0_LINE_IN0;
#else
		channel_id = AUDIO_ANALOG_MIC0;
#endif
	...
	case AUDIO_STREAM_FM:
		...
		break;
	case AUDIO_STREAM_OKMIC_FM:
		channel_id = AUDIO_ANALOG_FM0_AMIC0;
		break;

	case AUDIO_STREAM_OKMIC_LINE_IN:
		channel_id = AUDIO_ANALOG_AMIC0_LINE_IN0;
		break;
	}
...
int audio_policy_get_record_input_left_gain(u8_t stream_type)
{
	int gain = 0;/* odb */
...
int audio_policy_get_record_input_right_gain(u8_t stream_type)
{
	int gain = 0;/* odb */
...
int audio_policy_get_da_volume(u8_t stream_type, int volume_level)
{
...
int audio_policy_get_reduce_threshold(u8_t stream_type, u8_t exf_stream_type)
{
	...
	case AUDIO_STREAM_VOICE:
		if (system_check_low_latencey_mode())
			reduce_threshold = start_threshold - start_threshold / 3;
		else
			reduce_threshold = 44;
		break;
...
int audio_policy_get_increase_threshold(u8_t stream_type, u8_t exf_stream_type)
{
	int start_threshold = audio_policy_get_out_input_start_threshold(stream_type, exf_stream_type);
	...
	case AUDIO_STREAM_MUSIC:
		if (system_check_low_latencey_mode()) {
			if(btif_tws_get_dev_role() == BTSRV_TWS_MASTER) {
				increase_threshold = 110;
			} else {
				increase_threshold = 70;
			}
		} else {
			increase_threshold = 210;
		}
		break;
	case AUDIO_STREAM_LINEIN:
	case AUDIO_STREAM_USOUND:
...
int audio_policy_register(const struct audio_policy_t *policy)
{
	...
	media_effect_load("music.efx", &size, AUDIO_STREAM_MUSIC);
...
int audio_policy_get_usound_mode(void)
{
#ifdef CONFIG_USOUND_MEETING_MODE
	return USOUND_MODE_MEETING;
#else
	return USOUND_MODE_DEFAULT;
#endif
}

int audio_policy_get_aec_ref_mode(void)
{
#ifdef CONFIG_AEC_REF_HW_MODE
	return AEC_REF_MODE_HW;
#else
	return AEC_REF_MODE_SOFT;
#endif
}
