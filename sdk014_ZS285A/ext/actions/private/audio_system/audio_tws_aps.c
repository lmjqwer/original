...
static int _audio_is_ready(void *aps_monitor)
{
	...
	if (stream_get_length(stream) >= 1024) {
...
static int _audio_get_sample_rate(void *aps_monitor)
{
	aps_monitor_info_t *handle = (aps_monitor_info_t *)aps_monitor;
	if (handle->sample_rate == 44) {
		return 44;
	} else if (handle->sample_rate == 48) {
		return 48;
	} else {
		return 0;
	}
...
static int _audio_get_cache_time(void *aps_monitor, int *max_pcm, int *min_pcm)
{
	aps_monitor_info_t *handle = (aps_monitor_info_t *)aps_monitor;

	*max_pcm =  handle->aps_reduce_water_mark;
	*min_pcm =  handle->aps_increase_water_mark;

	return handle->cache_pcm_time;
}
...
static media_runtime_observer_t audio_observer = {
	...
	.get_sample_rate = _audio_get_sample_rate,
	.get_cache_time = _audio_get_cache_time,
};
