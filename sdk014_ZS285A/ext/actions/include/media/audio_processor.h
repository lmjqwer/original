...
#define MAX_AUDIO_IN_CHANNELS (3)
...
typedef enum {
	...
	USOUND_PREPROCESSOR_MODULE_ID = MAKE_AUDIO_PROCESSOR_MODULE_ID('U', 'S', 'D', 'R'),

	USOUND_POSTPROCESSOR_MODULE_ID = MAKE_AUDIO_PROCESSOR_MODULE_ID('U', 'S', 'D', 'O'),

	OKMIC_PREPROCESSOR_MODULE_ID  = MAKE_AUDIO_PROCESSOR_MODULE_ID('O', 'K', 'M', 'C'),
...
typedef struct audio_preprocessor_init_param {
	...
	u8_t attach_stream_type;/* okmic attach stream type of the media */
	...
	struct acts_ringbuf *inbufs[MAX_AUDIO_IN_CHANNELS];
...
typedef struct audio_postprocessor_init_param {
	...
	u8_t format;	/* format type to do dae extern effect */
...
typedef struct audio_processor_module {
	...
	/** Module ops: dump session info */
	int (*dump_session)(audio_processor_t *handle);
...
int audio_processor_session_dump(void *handle);
/**
