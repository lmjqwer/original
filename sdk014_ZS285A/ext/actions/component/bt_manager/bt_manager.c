...
#ifdef CONFIG_USER_DELETE_BACKPLAY_BT_TTS
#define EXT_APP_ID_BTMUSIC			"btmusic"
#endif
...
int bt_manager_set_status(int state)
{
	...
	#ifdef CONFIG_USER_DELETE_BACKPLAY_BT_TTS
			if(!strcmp(app_manager_get_current_app(), EXT_APP_ID_BTMUSIC)) {
...
int bt_manager_set_afh_host_channel(u8_t *channel)
{
	...
	return btif_set_afh_host_channel(channel);
}
