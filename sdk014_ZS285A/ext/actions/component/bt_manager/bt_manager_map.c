...
static void time_map_connect_failed_cb(u8_t app_id)
{
	SYS_LOG_INF("%d\n", app_id);
}

static void time_map_connected_cb(u8_t app_id)
{
	SYS_LOG_INF("%d\n", app_id);
}

static void time_map_disconnected_cb(u8_t app_id)
{
	SYS_LOG_INF("%d\n", app_id);
}

static void time_map_set_path_finished(u8_t app_id)
{
	SYS_LOG_INF("%d\n", app_id);
...
static int _bt_map_set_time(u8_t *time)
{
	u8_t tmp_buf[5];
	struct rtc_time tm;
...
static void time_map_result_cb(u8_t app_id, struct mgr_map_result *result, u8_t size)
{
	int i;
