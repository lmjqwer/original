...
static int system_okmic_solf_volume_set(int stream_type, int volume)
{
	int ret = 0;
	u32_t da_volume = 0;
	...
	if (!okmic_ctx_s || !okmic_ctx_s->player) {
		SYS_LOG_ERR("player null\n");
		return -ESRCH;
	}

	da_volume = audio_policy_get_da_volume(stream_type, volume);

	media_player_set_okmic_music_vol(okmic_ctx_s->player, da_volume);

	SYS_LOG_INF("volume: %d %d ret %d ok\n", volume, da_volume, ret);
...
#ifdef CONFIG_OK_MIC_APP
	system_okmic_solf_volume_set(stream_type, volume);
#else
...
#endif
#ifdef CONFIG_OK_MIC_APP
	if (strncmp(app_manager_get_current_app(), "btcall", strlen("btcall")) == 0
		|| strncmp(app_manager_get_current_app(), "btmusic", strlen("btmusic")) == 0) {
