 ...
 */
static void phy_spdifrx_prepare_enable(void)
{
...
#else
			/* SPDIFRX CLOCK oughts to range from (8 x 128 x fs) to (20 x 128 x fs)*/
			if (priv->sample_rate <= SAMPLE_RATE_32KHZ) {
		        sys_write32(((sys_read32(CMU_SPDIFRXCLK) & (~0x03)) \
		            | (3 << 0)), CMU_SPDIFRXCLK);
			} else if (priv->sample_rate < SAMPLE_RATE_64KHZ) {
		        sys_write32(((sys_read32(CMU_SPDIFRXCLK) & (~0x03)) \
		            | (1 << 0)), CMU_SPDIFRXCLK);
			} else {
		        sys_write32((sys_read32(CMU_SPDIFRXCLK) & (~0x03)), CMU_SPDIFRXCLK);
			}
#endif
		}
