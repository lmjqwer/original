...
#define DAC_MAX_PCM_VALUE                                      (0x7FFFFFFF)
#define DAC_ANTIPOP_PROCESS_SAMPLES                            (9600) /* 200ms in 48kfs stereo mode */
#define DAC_ANTIPOP_PROCESS_RAMP_STEP                          (0xFFFFFFFF / DAC_ANTIPOP_PROCESS_SAMPLES)
#define DAC_ANTIPOP_PROCESS_ON_DATA_CNT                        (2 * 48)
#define DAC_ANTIPOP_PROCESS_OFF_DATA_CNT                       (5 * 48)
...
{
	struct acts_audio_dac *dac_reg = get_dac_base();
	dac_reg->fifoctl &= ~DAC_FIFOCTL_DAFRT;
...
#ifdef CONFIG_DAC_SINGLE_END_LAYOUT
static void dac_single_end_on_antipop(void)
{
	struct acts_audio_dac *dac_reg = get_dac_base();
	u32_t reg, ramp_data, ramp_samples = DAC_ANTIPOP_PROCESS_SAMPLES;
	u8_t i;

	/* Enable DAC clock gate */
	acts_clock_peripheral_enable(CLOCK_ID_AUDIO_DAC);

	audio_samplerate_dac_set(SAMPLE_RATE_48KHZ);
	...
	dac_reg->digctl |= (DAC_DIGCTL_OSRDA(1) | DAC_DIGCTL_INTFRS(3));
	dac_reg->digctl |= (DAC_DIGCTL_DDEN | DAC_DIGCTL_ENDITH);

	/* 2.switch dacfifo input to cpu */
	reg = dac_reg->fifoctl;
	reg |= DAC_FIFOCTL_DAFRT; /* enable fifo */
	reg &= ~(DAC_FIFOCTL_DAFEDE | DAC_FIFOCTL_DAFEIE); /* disable drq and irq */
	reg &= ~DAC_FIFOCTL_DAFIS_MASK; /* set to cpu */
	dac_reg->fifoctl = reg;

	/* 3.send max pcm data to fifo0 and delay2ms */
	for (i = 0; i < DAC_ANTIPOP_PROCESS_ON_DATA_CNT; i++) {
		while(!(dac_reg->stat & DAC_STAT_DAFEIP)) {
			; /* wait DAC FIFO empty IRQ PD */
		}
		dac_reg->fifodat = DAC_MAX_PCM_VALUE;
		dac_reg->fifodat = DAC_MAX_PCM_VALUE;
		dac_reg->stat |= DAC_STAT_DAFEIP; /* clear empty irq pending */
	}

	/* wait PCMBUF empty */
	while((dac_reg->pcm_buf_stat & PCM_BUF_STAT_PCMBS_MASK) < (DAC_PCMBUF_MAX_CNT - 1)) {
		;
	}

	if ((dac_reg->pcm_buf_stat & PCM_BUF_STAT_PCMBS_MASK) == (DAC_PCMBUF_MAX_CNT - 1))
		k_busy_wait(10);
	...
	/* 5.dac digital begin to send max ramp-data untill min */
	ramp_data = DAC_MAX_PCM_VALUE;
	while (ramp_samples > 0) {
		while(!(dac_reg->stat & DAC_STAT_DAFEIP)) {
			; /* wait DAC FIFO empty IRQ PD */
		}
		while((dac_reg->pcm_buf_stat & PCM_BUF_STAT_PCMBS_MASK) < (DAC_PCMBUF_MAX_CNT - 1)) {
		...
		if ((dac_reg->pcm_buf_stat & PCM_BUF_STAT_PCMBS_MASK) == (DAC_PCMBUF_MAX_CNT - 1))
			k_busy_wait(10);
		dac_reg->fifodat = ramp_data; /* effective bit is 20,L */
		dac_reg->fifodat = ramp_data; /* R */
		dac_reg->stat |= DAC_STAT_DAFEIP; /* clear empty irq pending */
		ramp_data -= DAC_ANTIPOP_PROCESS_RAMP_STEP;
		ramp_samples--;
	}
	...
	dac_reg->anactl1 |= DAC_ANACTL1_SMCCKS_1KHZ; /* set to 1KHz */
...
void dac_single_end_off_antipop(void)
{
	struct acts_audio_dac *dac_reg = get_dac_base();
	u32_t reg, ramp_data, ramp_samples = DAC_ANTIPOP_PROCESS_SAMPLES;
	u8_t i;
	...
	/* 2.enable dac digital and switch dacfifo input to cpu*/
	dac_reg->digctl |= (DAC_DIGCTL_OSRDA(1) | DAC_DIGCTL_INTFRS(3));
	dac_reg->digctl |= (DAC_DIGCTL_DDEN | DAC_DIGCTL_ENDITH);
	...
	for (i = 0; i < DAC_ANTIPOP_PROCESS_OFF_DATA_CNT; i++) {
		while(!(dac_reg->stat & DAC_STAT_DAFEIP)) {
			; /* wait DAC FIFO empty IRQ PD */
		}
		while((dac_reg->pcm_buf_stat & PCM_BUF_STAT_PCMBS_MASK) < (DAC_PCMBUF_MAX_CNT - 1)) {
	...
	/* 4.dac digital begin to send max ramp-data untill min */
	ramp_data = DAC_MAX_PCM_VALUE;
	while (ramp_samples > 0) {
		while(!(dac_reg->stat & DAC_STAT_DAFEIP)) {
			; /* wait DAC FIFO empty IRQ PD */
	...
	/* 5.disable pa,bcdisch,atprcen,sw1,dac ana,dac dig */
...
#ifdef CONFIG_DAC_DIFFERENCTIAL_LAYOUT
static void dac_differencial_on_antipop(void)
{
	int ramp_data = 0x7FFFF, max_pcm = 0x7FFFF, min_pcm = 0x80001, i;
	u32_t ramp_step = 4096*4;
	struct acts_audio_dac *dac_reg = get_dac_base();
	...
	dac_pcmbuf0_clk_enable();
	//dac_pcmbuf1_clk_enable(); /* PCMBUF1 already used by algorithm stack */

	dac_mute_control(false);

	/* 1.playback mute, all bias en, dac ana/dac dig/pa en, and set to diff mode */
	dac_reg->anactl0 |= DAC_ANACTL0_BIASEN; /* enable DAC + PA BIAS */
	dac_reg->anactl1 &= ~(DAC_ANACTL1_DPBMLP | DAC_ANACTL1_DPBMLN);
	dac_reg->anactl1 &= ~DAC_ANACTL1_DPBMRP;

	dac_reg->anactl0 &=~DAC_ANACTL0_MODE_SETTING_MASK;
	dac_reg->anactl0 |= DAC_ANACTL0_MODE_SETTING(0xD); /* DIFF 1.6vpp */
	...
	/* enable nagative pa output-stage */
	...
	dac_reg->vol_lch = 0xBF;
	dac_reg->vol_rch = 0xBF;

	/* 2.switch dacfifo0 input to cpu */
	dac_reg->fifoctl = 1;

	/* 3.send max pcm data to fifo and delay2ms */
	for (i = 0; i < (2*48); i++) {
		/* wait empty IRQ pending */
		while (!(dac_reg->stat & DAC_STAT_DAFEIP)) {
			;
		}

		dac_reg->fifodat = max_pcm;
		dac_reg->fifodat = max_pcm;

		/* clear empty IRQ pending */
		dac_reg->stat |= DAC_STAT_DAFEIP;
	}
	...
	/* 4.enable loop2 and SW2 connect for LP and RP */
	...
	while (ramp_data > min_pcm) {
		/* wait fifo has a pair samples */
		while ((dac_reg->stat & DAC_STAT_DAFS_MASK) < 6);
		dac_reg->fifodat = ramp_data << 12; /* effective bit is 20,L */
		dac_reg->fifodat = ramp_data << 12; /* effective bit is 20,R */
		ramp_data -= ramp_step;
		/* wait pcmbuf empty */
		while((dac_reg->pcm_buf_stat & PCM_BUF_STAT_PCMBS_MASK) < (DAC_PCMBUF_MAX_CNT - 1)) {
			;
	...
	/* 7.enable sw1 and pa outputstage, disable sw2 */
	dac_reg->anactl0 |= (DAC_ANACTL0_SW1LP | DAC_ANACTL0_SW1RP);
	dac_reg->anactl0 |= (DAC_ANACTL0_PALPOSEN | DAC_ANACTL0_PARPOSEN); /* en positive pa output-stage */
	dac_reg->anactl1 &= ~(DAC_ANACTL1_ATPSW2LP | DAC_ANACTL1_ATPSW2RP);

	/* 8.send ramp data for disable atprcen */
	ramp_data = 0;
	...
	/* 9.disable loop2 and atprcen and atprcen2 */
	dac_reg->anactl1 &= ~(DAC_ANACTL1_LP2LPEN | DAC_ANACTL1_LP2RPEN);
	dac_reg->anactl1 &= ~(DAC_ANACTL1_ATPSW2LP | DAC_ANACTL1_ATPSW2RP);
	dac_reg->anactl1 &= ~(DAC_ANACTL1_ATPRC2EN_LP | DAC_ANACTL1_ATPRC2EN_RP);
	dac_reg->anactl1 &= ~(DAC_ANACTL1_ATPRCEN_LP | DAC_ANACTL1_ATPRCEN_RP);
	...
	dac_reg->anactl1 &= ~DAC_ANACTL1_SMCCKS_MASK;
	dac_reg->anactl1 |= (0x2 << DAC_ANACTL1_SMCCKS_SHIFT); /* set 8KHz soft mute clk */
	dac_reg->anactl1 |= DAC_ANACTL1_SMCEN;
 ...
 * @return void
 */
static void dac_poweron_antipop_process(void)
{
	u32_t start_time;
	start_time = k_uptime_get();
#ifdef CONFIG_DAC_SINGLE_END_LAYOUT
	dac_single_end_on_antipop();
#elif CONFIG_DAC_DIFFERENCTIAL_LAYOUT
	dac_differencial_on_antipop();
#endif
	SYS_LOG_INF("poweron antipop process take %dms", k_uptime_get_32() - start_time);
}
 ...
 * @return void
 */
static void dac_poweroff_antipop_process(void)
{
	u32_t start_time;
	start_time = k_uptime_get();
#ifdef CONFIG_DAC_SINGLE_END_LAYOUT
	dac_single_end_off_antipop();
#endif
	SYS_LOG_INF("poweroff antipop process take %dms", k_uptime_get_32() - start_time);
}
#endif
