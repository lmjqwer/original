...
static int uart_acts_fifo_switch(struct device *dev, u32_t is_tx, u32_t fifo_type)
{
    ...
    if(is_tx){
        ...
        if(fifo_type == UART_FIFO_DMA){
            //dma
            tmp |= UART_CTL_RX_FIFO_THREHOLD_1BYTE;  // UART_CTL_RX_FIFO_THREHOLD_8BYTES
        }else{
...
int acts_dma_remain_size(int handle);
...
static int uart_acts_dma_recv_remain_len(struct device *dev)
{
    ...
    remain_size = acts_dma_remain_size(uart_data->rev_chan);
...
static void uart_baud_rate_set(struct device *dev)
{
    int uart_div;
