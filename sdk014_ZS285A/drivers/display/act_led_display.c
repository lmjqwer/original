...
static int led_display_init(struct device *dev)
{
	...
	for (i = 0; i < led->led_dot_num; i ++) {
		...
		if (dot->led_pwm) {
			...
			if(dot->active_level == 0)
				gpio_pin_configure(dot->op_dev, dot->led_pin, GPIO_DIR_OUT | GPIO_POL_INV);			//低有效用这条
			else
				gpio_pin_configure(dot->op_dev, dot->led_pin, GPIO_DIR_OUT);						//高有效用这条
		}
	    if (!dot->op_dev) {
