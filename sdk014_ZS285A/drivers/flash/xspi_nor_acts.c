...
//#define CONFIG_SPINOR_TEST_CACHE_READ
//#define CONFIG_SPINOR_TEST_SPI0_RW_SPEED
//#define CONFIG_SPINOR_TEST_SPI0_READ_POWER
//#define CONFIG_SPINOR_TEST_SPI0_WRITE_POWER
...
u8_t spinor_test_delaychain(void)
{
	...
	for (freq = 6; freq <= 234; freq += 6) {
        ...
        //COREPLL,SYSCLK,SPI0CLK
        printk("COREPLL:%x, SYSCLK:%x, SPI0CLK:%x\n", sys_read32(0xC0000114), sys_read32(0xC0001000), sys_read32(0xC0001020));
#endif
...
#ifdef CONFIG_SPINOR_TEST_CACHE_READ
u32_t spinor_test_cache_read(u32_t test_id)
{
    u32_t acc = 0, i;
    u32_t cache_base = CONFIG_FLASH_BASE_ADDRESS;
    u32_t cache_size = 0xF4000; /* 1M - 16K(mbrec) - 32K(vram) */
    u32_t *cache_addr;

    printk("spinor_test_cache_read[%d] start\n", test_id);

    soc_freq_set_cpu_clk(0, 180);
    printk("set cpu freq : %d\n", 180);
#if defined(CONFIG_SOC_SERIES_WOODPECKER) || defined(CONFIG_SOC_SERIES_WOODPECKERFPGA)
    //COREPLL,SYSCLK,SPI0CLK
    printk("COREPLL:%x, SYSCLK:%x, SPI0CLK:%x\n", sys_read32(0xC0000114), sys_read32(0xC0001000), sys_read32(0xC0001020));
    ...
    for (i = 0; i < cache_size; i += 32) {
        cache_addr = (u32_t *)(cache_base + i);
        acc += *cache_addr;
    }
    printk("read complete, 0x%x\n", acc);

    return 0;
}
#endif

extern char _media_memory_start[];

#ifdef CONFIG_SPINOR_TEST_SPI0_RW_SPEED
u32_t spinor_test_spi0_rw_speed(void)
{
    struct device *test_nor_dev = device_get_binding(CONFIG_XSPI_NOR_ACTS_DEV_NAME);
    u32_t nor_rw_base = (u32_t)0xff000; /* 1M last 4kbytes */
    u8_t *nor_rw_buf = (u8_t *)(&_media_memory_start);
    int i, j, slice_size;

    printk("spinor_test_spi0_rw_speed start %p\n", nor_rw_buf);
    ...
    slice_size = 0x1000;
    for (j = 0; j < 0x1000/slice_size; j++)
    {
        printk("write start : %d us\n", BROM_TIMESTAMP_GET_US());
        xspi_nor_write(test_nor_dev, nor_rw_base + slice_size*j, nor_rw_buf + slice_size*j, slice_size);
        printk("write complete : %d us\n", BROM_TIMESTAMP_GET_US());
    }

    memset(nor_rw_buf, 0, 0x1000);
    printk("read start : %d us\n", BROM_TIMESTAMP_GET_US());
    xspi_nor_read(test_nor_dev, nor_rw_base, nor_rw_buf, 0x1000);
    printk("read complete : %d us\n", BROM_TIMESTAMP_GET_US());

    for (i = 0; i < 0x1000; i++) {
        if (nor_rw_buf[i] != (i&0xff)) {
            printk("read, data error at %d\n", i);
            break;
        }
    }

    xspi_nor_erase(test_nor_dev, nor_rw_base, 0x1000);
    xspi_nor_read(test_nor_dev, nor_rw_base, nor_rw_buf, 0x1000);
    for (i = 0; i < 0x1000; i++) {
        if (nor_rw_buf[i] != 0xff) {
            printk("erase, data error at %d\n", i);
            break;
        }
    }
...
#ifdef CONFIG_SPINOR_TEST_SPI0_READ_POWER
u32_t spinor_test_spi0_read_power(void)
{
    struct device *test_nor_dev = device_get_binding(CONFIG_XSPI_NOR_ACTS_DEV_NAME);
    u32_t nor_rw_base = (u32_t)0xff000; /* 1M last 4kbytes */
    u8_t *nor_rw_buf = (u8_t *)(&_media_memory_start);
    u32_t start_ts = BROM_TIMESTAMP_GET_US();
    int i;

    printk("spinor_test_spi0_read_power start %p\n", nor_rw_buf);
    ...
    xspi_nor_write(test_nor_dev, nor_rw_base, nor_rw_buf, 0x1000);

    memset(nor_rw_buf, 0, 0x1000);

    while (1) {
        #if 1 /* I at open - I at close */
        xspi_nor_read(test_nor_dev, nor_rw_base, nor_rw_buf, 0x1000);
        for (i = 0; i < 0x1000; i++) {
            if (nor_rw_buf[i] != (i&0xff)) {
                printk("read, data error at %d\n", i);
        ...
        #endif
        if (BROM_TIMESTAMP_GET_US() - start_ts >= 1000000) {
            printk("read power test...\n");
            start_ts = BROM_TIMESTAMP_GET_US();
        }
#if defined(CONFIG_SOC_SERIES_WOODPECKER) || defined(CONFIG_SOC_SERIES_WOODPECKERFPGA)
        //watchdog clear
        sys_write32(sys_read32(0xC012001C) | 0x01, 0xC012001C);
#endif
    }
...
#ifdef CONFIG_SPINOR_TEST_SPI0_WRITE_POWER
u32_t spinor_test_spi0_write_power(void)
{
    struct device *test_nor_dev = device_get_binding(CONFIG_XSPI_NOR_ACTS_DEV_NAME);
    u32_t nor_rw_base = (u32_t)0xff000; /* 1M last 4kbytes */
    u8_t *nor_rw_buf = (u8_t *)(&_media_memory_start);
    ...
    printk("spinor_test_spi0_write_power start %p\n", nor_rw_buf);
...
#ifdef CONFIG_SPINOR_TEST_CACHE_READ
    spinor_test_cache_read(1);
    spinor_test_cache_read(2);
    spinor_test_cache_read(3);
#endif

#ifdef CONFIG_SPINOR_TEST_SPI0_RW_SPEED
    spinor_test_spi0_rw_speed();
#endif

#ifdef CONFIG_SPINOR_TEST_SPI0_READ_POWER
    spinor_test_spi0_read_power();
#endif

#ifdef CONFIG_SPINOR_TEST_SPI0_WRITE_POWER
    spinor_test_spi0_write_power();
#endif
