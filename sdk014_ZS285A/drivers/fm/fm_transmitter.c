...
int fm_transmitter_aout_chan_set(struct device *fm_dev, bool playing)
{
	struct acts_fm_device_data *fm_info = fm_dev->driver_data;
	...
	if (!sub_dev->aout_chan_set) {
		return -EINVAL;
	}
	
	return sub_dev->aout_chan_set(sub_dev, playing);
}
#endif

#ifdef CONFIG_USER_UPDATE_TXPOWER_MODE	
int fm_transmitter_aout_txpower_set(struct device *fm_dev, u8_t pa_current, u8_t pa_gain)
{
	struct acts_fm_device_data *fm_info = fm_dev->driver_data;
	...
	if (!sub_dev->aout_txpower) {
		return -EINVAL;
	}
	
	return sub_dev->aout_txpower(sub_dev, pa_current, pa_gain);
}
#endif
