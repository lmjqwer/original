...
#if defined(CONFIG_USB_UART_CONSOLE)
		if (uart_usb_is_enabled() && uart_irq_tx_ready(uart_console_dev)) {
...
void console_input_init(void)
{
	...
	if (!uart_console_dev) {
		if (uart_usb_is_enabled())
			uart_console_dev = device_get_binding(CONFIG_CDC_ACM_PORT_NAME);
		else
...
#if defined(CONFIG_USB_UART_CONSOLE)
static int usb_console_init(struct device *arg)
{
	uart_usb_early_init();
	return 0;
}
SYS_INIT(usb_console_init, POST_KERNEL, 10);
#endif
