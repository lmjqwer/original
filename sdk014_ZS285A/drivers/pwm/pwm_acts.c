...
static int pwm_acts_clr_pinmux(struct device *dev, u32_t chan)
{
	int i, j;
    for (i = 0; i < ARRAY_SIZE(board_led_pin_config); i++) {
		for (j = 0; j < ARRAY_SIZE(pwmledmaps); j++) {
			if ((chan == pwmledmaps[j].led_pwm)
				&& (board_led_pin_config[i].pin_num == pwmledmaps[j].led_pin)) {
				acts_pinmux_set(board_led_pin_config[i].pin_num, 0);
				return 0;
			}
		}
    }
...
static int pwm_acts_set_program_mode(struct device *dev, u32_t chan, pwm_program_ctrl_t *ctrl)
{
	...
	/* setup pwm parameters */
	if (START_VOLTAGE_HIGH == ctrl->flag)
		pwm_chan->ctrl = PWM_CTRL_POL_SEL_HIGH | PWM_CTRL_MODE_SEL_PROGRAM;
	else
		pwm_chan->ctrl = PWM_CTRL_MODE_SEL_PROGRAM;
	...
	/* FIXME: IC issue
	  * The first few PWM phases are bad and shall skip those by PWM pinmux switch.
	  */
	k_busy_wait(100);
	if (pwm_acts_set_pinmux(dev, chan)) {
		SYS_LOG_DBG("set pwm pinmux error");
	...
	if (!ctrl->reload_en) {
		err = k_sem_take(&data->trans_done, K_FOREVER);
		if (err)
			ret = -ETIMEDOUT;
		pwm_chan->ctrl = 0;
		dmactrl->dmactl &= ~PWM_DMA_CTL_START;
		pwm_acts_clr_pinmux(dev, chan);
	}
...
int pwm_acts_init(struct device *dev)
{
	...
	/* init PWM clock
	  * By default: clock source: 32K, div= / 4, pwm clock fs 8KHz period 125us
	  */
	for (i = 0; i < cfg->num_chans; i++) {
		sys_write32((CONFIG_PWM_CLK_SRC << CMU_PWMCLK_CLKSEL_SHIFT) \
					| CMU_PWMCLK_CLKDIV(CONFIG_PWM_CLK_DIV), \
					cfg->pwmclk_reg + 4 * i);
	}
