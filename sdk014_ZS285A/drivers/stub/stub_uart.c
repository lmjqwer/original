...
static int stub_uart_rw_try(struct device *dev, uart_session_request_t *q)
{

	int ret = 0;
	ret = uart_session_rw(q);
	return ret;

}

static int stub_uart_w_try(uart_session_request_t *q)
{
	int ret = 0;

	ret = uart_session_w_only(q);
	if (ret > 0) {
		return 0;
	} else {
		return -1;
...
static int stub_uart_read(struct device *dev, u16_t opcode, u8_t *buf, u32_t len)
{
	...
	cmd.reserv = 0;
	cmd.data_len = len;        // alway 0

	q->request = STUB_OP_READ;
	q->timeout = stub->stub_uart_timeout;
	q->debug_mode = stub->stub_uart_debug_mode;

	q->send_data = (u8_t *)&cmd;
	q->send_size = (u16_t)sizeof(cmd);

    q->recv_data = buf;
    q->recv_size = len;

	result = stub_uart_rw_try(dev, q);
...
static int stub_uart_write(struct device *dev, u16_t opcode, u8_t *buf, u32_t len)
{
	...
	stub_trans_cmd_t *cmd = buf ? (stub_trans_cmd_t *)buf : &zero_cmd;
	...
	printk("cmd %d %d %d %d %d\n", cmd->type, cmd->opcode, cmd->sub_opcode, cmd->reserv, cmd->data_len);
	q->request = STUB_OP_READ;
	q->timeout = stub->stub_uart_timeout;
	q->debug_mode = stub->stub_uart_debug_mode;

	q->send_data = (u8_t *)cmd;
	q->send_size = len + STUB_COMMAND_HEAD_SIZE;

	q->recv_data = NULL;
	q->recv_size = 0;

	result = stub_uart_w_try(q);
