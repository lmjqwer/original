...
#define CEC_RX_MAX_ITEMS (10)
#define CEC_RX_RINGBUF_SIZE (CEC_RX_MAX_ITEMS * sizeof(struct cec_msg))

static u8_t cec_rx_buffer[CEC_RX_RINGBUF_SIZE];
...
static u8_t cec_get_local_address(struct device *dev)
{
	const struct acts_cec_config *config = dev->config->config_info;
	struct cec_acts_controller *cec = config->base;

	return ((cec->cr & CEC_LOGICAL_ADDR_MASK) >> CEC_LOGICAL_ADDR_SHIFT);
}
...
#ifdef CEC_RW_DEBUG
	SYS_LOG_INF("cec tx: initiator:0x%x destination:0x%x result:%d", msg->initiator, msg->destination, ret);
	SYS_LOG_INF("cec tx buffer[%d]:", msg->len);
	print_buffer(msg->buf, 1, msg->len, 16, -1);
#endif
...
static int acts_cec_read(struct device *dev, struct cec_msg *msg, u32_t timeout_ms)
{
	...
	while (data->cbuf.length < sizeof(struct cec_msg)) {
		if ((k_uptime_get_32() - start_time) >= timeout_ms) {
			SYS_LOG_DBG("ecc rx wait timeout");
			ret = -ETIMEDOUT;
			goto out;
		}
		k_sleep(1);
	...
	ret = cbuf_read(&data->cbuf, msg, sizeof(struct cec_msg));
	if (ret != sizeof(struct cec_msg)) {
		SYS_LOG_ERR("cec rx cbuf error");
...
#ifdef CEC_RW_DEBUG
	SYS_LOG_INF("cec rx: initiator:0x%x destination:0x%x result:%d",
					msg->initiator, msg->destination, ret);
	if (!ret) {
		SYS_LOG_INF("cec rx buffer[%d]:", msg->len);
		print_buffer(msg->buf, 1, msg->len, 16, -1);
	}
#endif
...
int acts_cec_init(struct device *dev)
{
	...
	cbuf_init(&data->cbuf, cec_rx_buffer, sizeof(cec_rx_buffer));
...
void acts_cec_isr(void *arg)
{
	...
	if (cec->rxcr & CEC_RX_INT) {
		...
		if (cec->rxcr & CEC_RX_FIFO_OV) {
			data->state = CEC_STATE_ERROR;
			SYS_LOG_ERR("cec rx overflow");
			/* rx reset */
			cec_rx_reset(cec);
		} else {
			rx_msg.initiator = (cec->rxcr & CEC_INIT_ADDR_MASK) >> CEC_INIT_ADDR_SHIFT;

			/* check RX EOM */
			if (!(cec->rxcr & CEC_RX_EOM)) {
				SYS_LOG_ERR("RX EOM error");
				cec_rx_reset(cec);
				return ;
			}
			...
			if (cec->rxcr & CEC_RX_MTYPE)
				rx_msg.destination = 0xF;
			else
				rx_msg.destination = (cec->cr & CEC_LOGICAL_ADDR_MASK) >> CEC_LOGICAL_ADDR_SHIFT;
			...
			for (i = 0; i < rx_cnt; i++) {
				...
				rx_msg.buf[i] = cec->rxdr;
			}
			rx_msg.len = rx_cnt;

			if ((rx_msg.destination == 0xF) && (rx_msg.initiator == cec_get_local_address(dev))) {
				SYS_LOG_INF("ignore initiator:%d broadcast message", rx_msg.initiator);
			} else {
				cbuf_write(&data->cbuf, &rx_msg, sizeof(rx_msg));
			}
		}
	}
}
