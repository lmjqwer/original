...
struct k_delayed_work cpuload_monitor_work;

void cpuload_check(uint32_t interval)
{
	struct k_thread *thread_list = NULL;
	unsigned int key, ratio, running_cycles, time_us;
	uint32_t curr_time;
	...
	for (;thread_list != NULL ;thread_list = (struct k_thread *)thread_list->next_thread) {
		running_cycles = thread_list->running_cycles;
		curr_prio = k_thread_priority_get(thread_list);
		if (curr_prio != 15) {
			continue;
	...
	if (flag) {
		printk(" idea thread too low! \ntask\t\t prio\t run(us)\t %%cpu\n");
...
static void cpuload_monitor_callback(struct k_work *work)
{
    cpuload_check(2000);

	k_delayed_work_submit(&cpuload_monitor_work, 2000);
}

void cpuload_monitor_start()
{

	k_delayed_work_init(&cpuload_monitor_work, cpuload_monitor_callback);
	k_delayed_work_submit(&cpuload_monitor_work, 2000);
...
{
	k_delayed_work_cancel(&cpuload_monitor_work);
}
