...
static int _rett_stub_read(RETT_data_t *data, uint16_t opcode, void *buf, uint16_t size)
{
...
{
	int ret = _rett_stub_read(data, STUB_CMD_RETT_READ_KARAOK_PARAM, &data->Karaoke_para, sizeof(Karaoke_para_t));

	if (!ret) {
		if (data->Karaoke_para.echo_para.echoEnable) {
			media_player_okmic_ctx_s *okmic_ctx_s = get_okmic_player_ctx();
			echo_para_t echo_para = { 0 };

			if (!okmic_ctx_s || !okmic_ctx_s->player)
				return -1;

			echo_para.echo_enable = data->Karaoke_para.echo_para.echoEnable;
			echo_para.echo_delay = data->Karaoke_para.echo_para.echoDelay;
			echo_para.echo_decay = data->Karaoke_para.echo_para.echoDecay;
			echo_para.echo_lowpass_fc1 = data->Karaoke_para.echo_para.echoLowFrq1;
			echo_para.echo_lowpass_fc2 = data->Karaoke_para.echo_para.echoLowFrq2;
			echo_para.decay_times = data->Karaoke_para.echo_para.reserve[0];

			media_player_update_okmic_mic_bypass_param(okmic_ctx_s->player, data->Karaoke_para.bypass);
			media_player_set_okmic_mic_vol(okmic_ctx_s->player, data->Karaoke_para.echo_para.echoVoiceDecay);
			media_player_update_okmic_echo_param(okmic_ctx_s->player, &echo_para);
		...
		if (data->Karaoke_para.reverb_para.reverbEnable) {
			...
			reverb_para.mRoomSize = data->Karaoke_para.reverb_para.reverbRoomSize;
			reverb_para.mPreDelay = data->Karaoke_para.reverb_para.reverbPreDelay;
			reverb_para.mReverberance = data->Karaoke_para.reverb_para.reverbReverberance;
			reverb_para.mHfDamping = data->Karaoke_para.reverb_para.reverbHfDamping;
			reverb_para.mToneLow = data->Karaoke_para.reverb_para.reverbToneLow;
			reverb_para.mToneHigh = data->Karaoke_para.reverb_para.reverbToneHigh;
			reverb_para.mWetGain = data->Karaoke_para.reverb_para.reverbWetGain;
			reverb_para.mDryGain = data->Karaoke_para.reverb_para.reverbDryGain;
			reverb_para.mStereoWidth = data->Karaoke_para.reverb_para.reverbStereoWidth;
			reverb_para.mWetOnly = data->Karaoke_para.reverb_para.reverbWetOnly;
			media_player_update_okmic_mic_bypass_param(okmic_ctx_s->player, data->Karaoke_para.bypass);
			media_player_set_okmic_mic_vol(okmic_ctx_s->player, data->Karaoke_para.reverb_para.reverDecay);
			media_player_update_okmic_reverb_param(okmic_ctx_s->player, &reverb_para);
		}
	}
...
static int _rett_get_update_info_flag(RETT_data_t *data, stUpdate_info_t *stUpdate_info)
{
	return _rett_stub_read(data, STUB_CMD_RETT_READ_UPDATE_INFO_FLAG, stUpdate_info, sizeof(stUpdate_info_t));
