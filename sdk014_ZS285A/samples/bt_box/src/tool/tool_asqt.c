...
static const uint8_t asqt_tag_table[6] = {
	...
	/* capture tags */
	MEDIA_DATA_TAG_REF, MEDIA_DATA_TAG_MIC1,
...
static int _asqt_stub_upload_case_info(struct ASQT_data *data)
{
	...
	//sample rate type: 8k->0, 16k->1, 48k->2
	if (!(strcmp(APP_ID_BTCALL, app_manager_get_current_app()))) {
		p_config_info->sample_rate = (data->codec_id == CVSD_TYPE) ? 0 : 1;
	} else if (!(strcmp(APP_ID_USOUND, app_manager_get_current_app()))) {
		p_config_info->sample_rate = 2;
	}
...
static int _asqt_stub_effect_param_deal(struct ASQT_data *data)
{
	...
	u8_t stream_type = AUDIO_STREAM_VOICE;
	...
	if (!ret) {
		...
		data->asqt_para->sv_para.aec_prms.tail_length =
				audio_policy_get_record_channel_aec_tail_length(stream_type,
		...
		if (!data->has_effect_set) {
			medie_effect_set_user_param(stream_type, 0, data->asqt_para, sizeof(asqt_para_bin_t));
...
static int _asqt_stub_case_param_deal(struct ASQT_data *data)
{
	...
	if (!ret) {
		...
		system_volume_set(stream_type, case_param.sVolume, false);
		audio_system_set_stream_pa_volume(stream_type, case_param.sMaxVolume * 100);
		audio_system_set_microphone_volume(stream_type, case_param.sAnalogGain + case_param.sDigitalGain);
	}
...
{
	if (!(strcmp(APP_ID_BTCALL, app_manager_get_current_app())) ||
	!(strcmp(APP_ID_USOUND, app_manager_get_current_app()))) {
