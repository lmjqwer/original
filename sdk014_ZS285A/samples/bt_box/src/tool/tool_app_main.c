...
static int pc_connect_probe(void)
{
	...
	#ifndef CONFIG_TOOL_ASQT_USE_UART
	...
	struct device *serial_dev_stub = device_get_binding(CONFIG_STUB_DEV_UART_NAME);
	if (!serial_dev_stub) {
		SYS_LOG_ERR("serial dev not find");
		return -1;
	}

	ret = stub_open(serial_dev_stub, NULL);
	if (ret != 0) {
		SYS_LOG_INF("uart stub open fail %d\n", ret);
	}


	dev_stub.stub_dev = serial_dev_stub;
	dev_stub.open_flag = 1;

	while(1) {
		pc_type = 0;

		ret = stub_get_data(&dev_stub, STUB_CMD_OPEN, &pc_type, 1);
		printk("recv data pc_type:%d\n", pc_type);

		if (ret == 0) {
			SYS_LOG_INF("serial connect ");
			g_tool_data.dev_type = TOOL_DEV_TYPE_UART1;

			if (g_tool_data.dev_stub) {
				act_stub_close(g_tool_data.dev_stub);
				g_tool_data.dev_stub = &dev_stub;
			}


			g_tool_data.dev_stub->stub_dev = serial_dev_stub;
			g_tool_data.dev_stub->open_flag = 1;
			break;
		}


		ret = stub_get_data(g_tool_data.dev_stub, STUB_CMD_OPEN, &pc_type, 4);
		if (ret == 0) {
			SYS_LOG_INF("usb connect");
			g_tool_data.dev_type = TOOL_DEV_TYPE_USB;
			stub_close(serial_dev_stub);
			break;
		}
