...
void audio_input_start_play(void)
{
	...
	if (init_param.stream_type == AUDIO_STREAM_LINEIN) {
		...
		init_param.sample_rate = 48;
		init_param.capture_channels = 2;
		init_param.capture_sample_rate_input = audio_input->spdif_sample_rate? audio_input->spdif_sample_rate:48;
		init_param.capture_sample_rate_output = 48;
...
#ifdef CONFIG_TWS
	if ((init_param.capture_sample_rate_input == 88 ||init_param.capture_sample_rate_input == 96)
		&& bt_manager_tws_get_dev_role() != BTSRV_TWS_NONE) {
		media_player_set_drc_bypass(audio_input->player, 0);
	}
#endif
