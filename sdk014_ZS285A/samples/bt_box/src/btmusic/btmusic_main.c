	...
	while (!terminaltion) {
		if (receive_msg(&msg, thread_timer_next_timeout())) {
			...
			case MSG_EXIT_APP:
				...
				if(aduio_system_btmusic_to_btcall_flag(READ_BTCALL_FLAG_DATA, false) == false) {
					if (p_btmusic_app->playing) {
						bt_manager_avrcp_pause();
						p_btmusic_app->playing = 0;
						// p_btmusic_app->bt_last_playing = 0;
						bt_music_a2dp_stop_play();
						if (thread_timer_is_running(&p_btmusic_app->monitor_timer)) {
							thread_timer_stop(&p_btmusic_app->monitor_timer);
						}
