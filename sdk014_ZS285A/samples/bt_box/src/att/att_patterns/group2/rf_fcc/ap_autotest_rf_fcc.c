...
#ifdef OPEN_GET_BT_MAC
static void _get_bt_mac(uint8_t *mac)
{
	uint8_t mac_str[16], mac_bytes[6], i;
	int ret;

	ret = property_get(CFG_BT_MAC, mac_str, 15);
	if (ret < 12) {
		SYS_LOG_WRN("ret: %d", ret);
	} else {
		SYS_LOG_INF("BT MAC: %s", mac_str);
        str_to_hex(mac_bytes, mac_str, 12);
        for (i = 0; i < 6; i++) {
            mac[i] = mac_bytes[5-i];
        }
	}
...
#ifdef OPEN_GET_BT_MAC
    _get_bt_mac((uint8_t *)FCC_DRV_EXT_PARAMS_ADDR); /*6 bytes mac, low of lap first*/
#endif
