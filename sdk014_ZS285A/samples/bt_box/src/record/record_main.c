...
int recorder_prepare(char *dir)
{
	...
	if (check_file_dir_exists(p_recorder->dir)) {
		...
		res = record_update_file_bitmaps(p_recorder, p_recorder->dir);
		if (res == 0) {/*no record file and in record playback mode, need to start record*/
			if ((recorder_init_state == RECFILE_STATUS_PLAYING) || (recorder_init_state == RECFILE_STATUS_STOP)) {
				recorder_init_state = RECORDER_STATUS_START;
			}
		}
		SYS_LOG_INF("update bitmaps :%d us,files :%d\n", (k_cycle_get_32() - begin)/24, res);
	}
