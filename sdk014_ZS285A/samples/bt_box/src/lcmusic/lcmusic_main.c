...
static void lcmusic_bp_save_500ms_timer(struct thread_timer *ttimer, void *expiry_fn_arg)
{
	if((p_local_music->music_state == LCMUSIC_STATUS_PLAYING) && (++p_local_music->lc_bp_save_cnt >= LCMUSIC_BP_SAVE_TIME)) {
		p_local_music->lc_bp_save_cnt = 0;
...
#ifdef CONFIG_USER_LCMUSIC_BP_SAVE
	thread_timer_init(&p_local_music->lc_bp_save_timer, lcmusic_bp_save_500ms_timer, NULL);
	thread_timer_start(&p_local_music->lc_bp_save_timer, 500, 500);
#endif
...
#ifdef CONFIG_USER_LCMUSIC_BP_SAVE
	if (thread_timer_is_running(&p_local_music->lc_bp_save_timer))
		thread_timer_stop(&p_local_music->lc_bp_save_timer);
#endif
