...
#define SYNC_RECORD_DATA_PERIOD OS_MSEC(10)
...
#if CONFIG_OK_MIC_APP
static char recorder_srv_adpcm_cache[0x2000] _NODATA_SECTION(.recorder_srv_adpcm);
#else
static char recorder_srv_adpcm_cache[0x2000] _NODATA_SECTION(.recorder_srv_adpcm);
#endif
...
extern int record_srv_drop_consumed_data(io_stream_t handle, int len);
...
static void _recorder_service_observer_notify(u32_t event, void *data, u32_t len, void *user_data)
{
	...
	if (event == PLAYER_EVENT_OPEN) {
		if (init_param->stream_type != AUDIO_STREAM_TTS && init_param->attach_stream_type != AUDIO_STREAM_TTS ) {
			os_mutex_lock(&media_record_info_mutex, OS_FOREVER);
			if (need_record) {
				SYS_LOG_INF("new sample_rate=%d,channels=%d,old sample_rate=%d,channels=%d\n", init_param->sample_rate, init_param->channels, g_record_info.sample_rate, g_record_info.channels);
...
static void _recorder_srv_read_cache(void)
{
	...
	while (stream_get_length(p_recorder_service->cache_stream) >= RECORDER_SRV_READ_CACHE_LEN) {
		...
		if (stream_write(p_recorder_service->output_stream, temp_cache_buf, RECORDER_SRV_READ_CACHE_LEN) != RECORDER_SRV_READ_CACHE_LEN)
			SYS_LOG_ERR("lost data\n");

		record_srv_drop_consumed_data(p_recorder_service->cache_stream, RECORDER_SRV_READ_CACHE_LEN);
	...
	if (need_flush) {
		if (fs_manager_get_fsinfo(strncmp(p_recorder_service->dir, "SD:", strlen("SD:")) ? "USB:":"SD:", &fs) == 0) {
			disk_ioctl(fs->drv, CTRL_SYNC, 0);
		}
	}
...
void recorder_service_start_stop(u8_t is_stop)
{
	...
	if (!p_recorder_service) {
		if (strncmp(app_manager_get_current_app(), APP_ID_RECORDER, strlen(APP_ID_RECORDER)) == 0)
			return;
	/*lcmusic unable to do recorder when ok mic function open*/
	#ifdef CONFIG_OK_MIC_APP
		if (strncmp(app_manager_get_current_app(), APP_ID_SD_MPLAYER, strlen(APP_ID_SD_MPLAYER)) == 0
			|| strncmp(app_manager_get_current_app(), APP_ID_UHOST_MPLAYER, strlen(APP_ID_UHOST_MPLAYER)) == 0
			|| strncmp(app_manager_get_current_app(), APP_ID_NOR_MPLAYER, strlen(APP_ID_NOR_MPLAYER)) == 0
			|| strncmp(app_manager_get_current_app(), APP_ID_LOOP_PLAYER, strlen(APP_ID_LOOP_PLAYER)) == 0)
			return;
	#endif
	#ifndef CONFIG_OK_MIC_APP
