...
#ifdef CONFIG_USER_KEY_ENTER_DUT_MODE
	case MSG_RESETT_ENTER_DUT_MODE:
	{
		SYS_LOG_INF("--- press 10s enter dut mode ---\n");
		property_set_int(CFG_BT_TEST_MODE, 1);
        property_flush(CFG_BT_TEST_MODE);
		user_control_extern_pa_ctl(1, false);		//关闭功放
		system_power_reboot(REBOOT_REASON_RESET_DUT);
		break;
	}
#endif
