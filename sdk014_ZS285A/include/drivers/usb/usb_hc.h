...
/**
 *@use software to avoid hardware bug. controller send token packet
 * to control endpoint 0 in data transfer phase.Therefore, when the
 * control transmission phase ends, HCEP0CTRL is configured as an epnum
 * that we think device(udisk) will not use.
 */
int usb_hc_quirk_control(void);
